...
namespace Decoratid.Thingness.Idioms.Store.Decorations.Intercepting
{
    /// <summary>
    ...
    public class InterceptingDecoration : DecoratedStoreBase, IInterceptingStore, IHasHydrationMap
    {
        ...
        {
            return this.GetOperationIntercept.Perform(soId.ValueOf());
        }
        public override List<T> Search<T>(SearchFilter filter)
        {
            //execute it
            var list = this.SearchOperationIntercept.Perform(new Tuple<Type, SearchFilter>(typeof(T), filter).ValueOf());
        ...
        {
            this.CommitOperationIntercept.Perform(bag.ValueOf());
        }
